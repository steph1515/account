...
<body>
    <div>
        ...
        if(isset($_GET["txtdesc"]) && isset($_GET["txtamount"]) && $_GET["txtamount"] > 0) {
            ...
            $account = $_GET["txtname"];
            
            // Fetch the last balance
            $result = $conn->query("SELECT fldbal FROM tblledger WHERE fldname='$account' ORDER BY id DESC LIMIT 1");
            ...
            if($accountType == "asset" || $accountType == "ownersEquity") {
                $balance += $amount;
                $stmt = $conn->prepare("INSERT INTO tblledger (fldname, flddesc, flddebit, fldbal) VALUES (?, ?, ?, ?)");
            } else {
                $balance -= $amount;
                $stmt = $conn->prepare("INSERT INTO tblledger (fldname, flddesc, fldcredit, fldbal) VALUES (?, ?, ?, ?)");
            }
